...
namespace makcu {
    ...
    // Common baud rates to try (like MamboDMA)
    ...
    constexpr const char* MAKCU_SIGNATURE = "km.MAKCU";
    ...
    public:
        ...
        // Validate MAKCU signature by sending km.version() and checking response (simple sync approach like MamboDMA)
        bool validateSignature(int timeoutMs = 500) {
            ...
            try {
                ...
                // Send version command (simple write, no tracking)
                if (!serialPort->sendCommand("km.version()")) {
                    return false;
                }
                ...
                std::string response = serialPort->readString(256);
                
                // Check if response contains "km.MAKCU"
                return response.find(MAKCU_SIGNATURE) != std::string::npos;
            } catch (...) {
            }
            return false;
        }
    ...
    bool Device::connect(const std::string& port, bool useHighSpeed) {
        // Don't hold mutex during serial operations - serial close() joins listener thread
        ...
        // Switch to high-speed mode (optional - some devices don't support 4Mbaud)
        if (useHighSpeed) {
        ...
        // Update device info (lock mutex only for state update)
        {
    ...
    std::future<bool> Device::connectAsync(const std::string& port, bool useHighSpeed) {
        return std::async(std::launch::async, [this, port, useHighSpeed]() {
            return connect(port, useHighSpeed);
            });
    }

    void Device::disconnect() {
        // Don't hold mutex during close - serial close() joins listener thread
        if (!m_impl->connected.load()) {
            return;
        }
    ...
    bool Device::connectAutoDetect(const std::string& port) {
        // Don't hold mutex during brute-force - serial close() needs to join listener thread
        if (m_impl->connected.load()) {
            return true;
        }
        ...
        // Try each baud rate until we get a valid signature
        for (uint32_t baudRate : COMMON_BAUD_RATES) {
            // Close if open from previous attempt (no mutex - serial port has its own)
            if (m_impl->serialPort->isOpen()) {
                m_impl->serialPort->close();
                std::this_thread::sleep_for(std::chrono::milliseconds(100));
            }

            if (!m_impl->serialPort->open(targetPort, baudRate)) {
                continue;
            }

            std::this_thread::sleep_for(std::chrono::milliseconds(150));

            // Try to validate signature at this baud rate
            if (m_impl->validateSignature(400)) {
                // Found working baud rate! Try switching to high speed if not already at 4M
                if (baudRate != HIGH_SPEED_BAUD_RATE) {
                    // Send baud change command
                    m_impl->serialPort->write(BAUD_CHANGE_COMMAND);
                    m_impl->serialPort->flush();
                    std::this_thread::sleep_for(std::chrono::milliseconds(100));

                    // Close and reopen at high speed
                    std::string savedPort = m_impl->serialPort->getPortName();
                    m_impl->serialPort->close();
                    std::this_thread::sleep_for(std::chrono::milliseconds(100));

                    // Try high speed, fall back to working baud rate
                    if (!m_impl->serialPort->open(savedPort, HIGH_SPEED_BAUD_RATE)) {
                        // High speed failed, reopen at original baud rate
                        if (!m_impl->serialPort->open(savedPort, baudRate)) {
                            continue;
                        }
                    }
                }
                ...
                // Initialize device
                if (m_impl->initializeDevice()) {
                    // Now lock mutex for state update
                    std::lock_guard<std::mutex> lock(m_impl->mutex);
                    m_impl->deviceInfo.port = targetPort;
                    m_impl->deviceInfo.description = TARGET_DESC;
                    m_impl->deviceInfo.vid = MAKCU_VID;
